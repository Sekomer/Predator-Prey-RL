...
class QuadrotorFormationMARL(gym.Env):
    ...
    def __init__(self, n_agents=1, n_bots=2,
        ...
        self.z_lim = 5
        # we need to give this to make the systems obsevation shape
        self.n_obstacle = len(self.obstacle_points)
        self.n_obstacle_points = len(self.obstacle_points)
        self.n_static_obstacle_points = len(self.static_obstacle_points)
        ...
        self.obs_shape = ((self.max_drone_agents + self.max_tank_agents +
                           self.max_drone_bots + self.max_tank_bots)*3+(self.x_lim*self.y_lim*self.z_lim)+6*self.n_obstacle+6*self.n_static_obstacle_points,)
    ...
    def step(self, action):
       ...
       # total_indices = np.arange(self.uncertainty_grids.shape[0])
        ...
        for obstacle_ind in range(self.n_obstacle_points):
        ...
        for obstacle_ind in range(self.n_static_obstacle_points):
        ...
        exploration_learning_success = False
        ...
        if (np.sum(self.uncertainty_grid_visit == 0) < self.x_lim*self.y_lim*self.z_lim*0.3) & self.exploration_learning:
            ...
            exploration_learning_success = True
            done = True

        if self.current_step > self.cutoff_step_number:
            done = True
            print("Map failed succesfully in" + str(self.current_step)+"steps")
            reward_list -= 100
        ...
        if self.current_step < self.cutoff_step_number & done == True & (not exploration_learning_success):
            print("Probabily did crash somewhere in " +
                  str(self.current_step) + "steps or cant complete the track")
    ...
    def reset(self):
        ...
        self.uncertainty_grid_visit = np.zeros(
            (self.x_lim, self.y_lim, self.z_lim))
        print("Initialization of new environment")
        self.current_step = 0
